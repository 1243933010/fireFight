        ...
        <el-form-item label="" prop="input12">
          ...
          <div v-for="(item, index) in radioLabelList" :key="index">
            <div class="radio-item" @click="setIndex(index)">
              <div class="radio-class" :class="activeIndex == index ? 'active' : ''"></div>
              <span>{{ item.label }}</span>

            </div>
            <div class="radio-item-child" v-if="item.child.length > 0 && childRadioBool">
...
export default {
  ...
  verifyForm(callback){
    ...
    setIndex(ind) {
      ...
      console.log(this.radioLabelList)
    },
