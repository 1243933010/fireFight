<template>
  <div>
    ...
    <div class="list">
      <el-table row-key="id" default-expand-all :data="list" :header-cell-style="setTitle" style="width: 100%" border fit
      ...
      <el-pagination style="text-align: right;" :current-page="form.current_page" :page-sizes="[10, 20, 50, 100]"
                :page-size="form.per_page" :total="form.total"
                layout="total, sizes, prev, pager, next, jumper" @@size-change="handleSizeChange"
          @current-change="handleCurrentChange" />
    </div>

    <AddDialog ref="add" @updateData="()=>{form.current_page=1;query();}" />
  </div>
</template>
...
import { agentsList, agentDelete, departmentArr } from "@/api/project";
export default {
  ...
  methods: {
    ...
    async query() {
      let res = await agentsList(this.form);
      console.log(res);
      if (res.code == 200) {
    ...
    deleteItem(item) {
      this.$confirm("此操作将删除该数据, 是否继续?", "提示", {
        ...
        .then(async () => {
          let res = await agentDelete(item.id);
          console.log(res);
          if (res.code === 200) {
            this.$message({
              type: "success",
