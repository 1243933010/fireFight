  ...
  <div>
    ...
    <el-form ref="form" size="small" inline :model="form" label-width="90px" class="form-container">
      ...
      <el-form-item label="采购方式">
        <el-select clearable v-model="form.procurement_method" placeholder="请选择采购方式">
          <el-option v-for="(item, index) in procurementMethodSelect" :key="index" :label="item.label"
            :value="item.value" />
        </el-select>
      </el-form-item>
      ...
      <el-form-item label="申请时间">
        <el-date-picker clearable v-model="form.region" type="daterange" range-separator="至" start-placeholder="开始日期"
          ...
          <div class="item-con">
            <div class="item-con-left">
              <div class="item-con-left-o">
                <div>
                  <span class="label">项目编号:</span><span class="text">{{ item.no }}</span>
                </div>
                <div>
                  <span class="label">采购单位:</span><span class="text">{{ item.demand_department }}</span>
                </div>
                <div>
                  <span class="label">采购申请时间:</span><span class="text">{{ item.apply_date }}</span>
                </div>
              </div>
              <div class="item-con-left-t">
                <div>
                  <span class="label">项目类型:</span><span class="text">{{ item.type_text }}</span>
                </div>
                <div>
                  <span class="label">预算金额:</span><span class="text">{{ item.budget }}</span>
                </div>
                <div>
                  <span class="label">代理机构名称:</span><span class="text">{{ item.agent }}</span>
                </div>
              </div>
            </div>
            <div class="item-con-right">
              ...
              <div v-permission="['admin', 'project_registrar']" class="item-con-right-btn3" @click="deleteItem(item)">删除
              </div>
            </div>
          </div>
        </div>
    ...
    <div style="display: flex;justify-content: space-between;flex-direction: row-reverse;">
      <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
        :current-page="form.current_page" :page-sizes="[10, 20, 30, 40]" :page-size="form.per_page"
